...
struct FROAccountData
{
	...
	int32 AccountID = 0;
	FString Username;
	FString PasswordHash; // SHA256 hash
	FString Email;
	FDateTime CreatedAt;
...
/**
 * UROAuthSubsystem
 * Handles account registration, authentication with SHA256 password hashing,
 * and session token generation/validation.
 */
UCLASS()
...
public:
	 ...
	 * @param Password  The plaintext password (will be hashed with SHA256).
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Auth")
	bool ValidateSessionToken(const FString& SessionToken) const;
