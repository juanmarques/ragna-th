...
#include "ROAbility_LightningBolt.h"
#include "AbilitySystemComponent.h"
#include "RagnarokUE/Skills/ROAttributeSet.h"
#include "RagnarokUE/Combat/ROElementalSystem.h"

UROAbility_LightningBolt::UROAbility_LightningBolt()
{
	SkillID = 21;
	SkillName = FName("MG_LIGHTNINGBOLT");
	MaxSkillLevel = 10;
	SkillLevel = 1;
	SkillElement = EROElement::Wind;
...
void UROAbility_LightningBolt::OnCastComplete()
{
	...
	if (DamageSpec.IsValid())
	{
		...
		{
			DamageSpec.Data->SetSetByCallerMagnitude(ElementModTag, 1.0f); // Wind element
		}
	}
...
int32 UROAbility_LightningBolt::GetBoltCount() const
{
	return SkillLevel;
}

float UROAbility_LightningBolt::GetPerBoltDamageModifier() const
{
	return 1.0f;
}
